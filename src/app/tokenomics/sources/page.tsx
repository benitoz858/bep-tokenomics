...
const SOURCES = [
  {
    ...
    confidence: "high",
    items: [
      { name: "OpenRouter API", url: "https://openrouter.ai/api/v1/models", what: "Token pricing for frontier closed-source and aggregate open-source rows. Source label = 'OpenRouter' (default) on the model table.", frequency: "Daily (6AM UTC)" },
  ...
  },
  {
    ...
    confidence: "medium",
    items: [
      { name: "Salesforce Agentforce Pricing", url: "https://www.salesforce.com/agentforce/pricing/", what: "$2/conversation (legacy SKU). Flex Credits at $0.10/action is the recommended deployment for new enterprises (May 2025). Per-user licenses $125-650/user/month also available.", frequency: "Quarterly (earnings)" },
      { name: "Salesforce Q3 FY2026 Earnings", url: "https://investor.salesforce.com/news/news-details/2025/Salesforce-Delivers-Record-Third-Quarter-Fiscal-2026-Results/", what: "Agentforce + Data 360 combined ARR ~$1.4B, +114% YoY. Agentforce-only ARR >$540M, +330%. 3.2T tokens processed. 18,500 deals closed (~9,500 paid).", frequency: "Quarterly" },
  ...
  },
  {
    ...
    items: [
      ...
      { name: "TCO Multiplier (1.25x)", url: null, what: "Applied to raw GPU spot pricing on the Overview to account for storage, reliability, goodput losses, support, setup, debugging overhead. Derived from Nebius-SemiAnalysis TCO framework. The Margin Calculator now shows BOTH a sticker view and a TCO-adjusted view so the two pages reconcile.", frequency: "Static assumption" },
      { name: "Availability % (Ornn-overridden)", url: null, what: "When Ornn OCPI utilization data is available it overrides the offering-count availability. So 'H100 4%' means 96% utilization on traded compute, NOT 4% of advertised offerings free. The free/rented counts on the same card describe a different denominator (advertised NeoCloud offerings) and are kept for context.", frequency: "Daily" },
      { name: "Inference Margin Calculator", url: null, what: "GPU throughput estimates (BEP Research) × GPU cost ÷ tokens/sec = production cost/M tokens. Compared to API sell price for margin. Two views: 'Sticker' uses raw $/hr; 'TCO' applies the 1.25x multiplier to match the Overview.", frequency: "Daily" },
      { name: "Token Cost Waterfall", url: null, what: "Margin stack estimated from public pricing (high confidence) + estimated tokens-per-interaction (low-medium confidence) + input/output token-cost blend (70/30 default, overridable per-platform). Auto-refreshed quarterly public-company revenue from SEC EDGAR XBRL (see platform-disclosures.json). Manual `margins.notes` prose flagged STALE in the UI after 90 days.", frequency: "Quarterly (SEC XBRL); manual prose per earnings cycle" },
      { name: "Static reference snapshots", url: null, what: "Several reference files (cost-stack, nvidia-tiers, revenue-per-watt, tco-providers, gpu-throughput, constraints, inference-provider-margins, gpu-hardware-specs) are intentional snapshots tied to specific events (GTC keynote, hardware launches, lab disclosures). Each file carries a `cadence` field declaring its expected refresh interval. They are NOT dead data — they are reviewed against the event cycle that produces them.", frequency: "Per event (GTC, earnings, launch)" },
